...
def main():
    ...
    parser.add_option(
        '-d', '--desktopApplicationID', dest='desktopApplicationID',
        help='specify desktop application by ID.', metavar='Application_Name_ID')
    parser.add_option(
        '-p', '--palmApplicationID', dest='palmApplicationID',
        help='specify palm application by ID', metavar='Palm_Application_ID')
    parser.add_option(
        '-l', '--listApps', action='callback',
        help='print out a list of supported Palm application IDs and their corresponding Desktop Applications',
        callback=listSupportAppsCallBack)
    ...
    if Palm is None:
        parser.error((
            'Can only convert between between a Palm database and a desktop application.'
            'You do not seem to have specified a palm database. Palm databases have to end in .pdb to be recognized.'
        ))

    if desktop is None:
        parser.error((
            'Can only convert between between a Palm database and a desktop application.'
            'You do not seem to have specified a desktop application file.'
        ))
