...
class ProgectPalmDBXMLReaderObject(PalmDB.Plugins.BasePlugin.GeneralPalmDBXMLReaderObject):
    ...
    def parse_START_DOCUMENT(self, events, node, palmDatabaseObject):
        PalmDB.Plugins.BasePlugin.GeneralPalmDBXMLReaderObject.parse_START_DOCUMENT(
            self, events, node, palmDatabaseObject
        )
...
class ProgectRecord(PalmDB.Plugins.BasePlugin.DataRecord):
    ...
    *may not be changed* once the object is created. You need to call
    ...
    def _setProgectFirstRecordProperties(self):
        self.attributes = {
            'itemType': 'PROGRESS_TYPE', '_hasDuration': False, 'hasLink': False, '_hasDueDate': True,
    ...
    def _crackPayload(self, dstr):
        if len(dstr) < PRI.TaskAttrTypeStructSize:
            raise IOError(
                "Error: raw data passed in is too small; required (%d), available (%d)" %
                (PRI.TaskAttrTypeStructSize, len(dstr))
            )
        ...
        if self.attributes['_hasXB']:
            ...
            # XB Offset will always be two more than the size variable,
            # to account for the variable embedded in the structure
    ...
    def fromByteArrayTaskStandardFields(self, dstr):
        ...
        (priority, completed, dueDate) = struct.unpack(
            PRI.TaskStandardFieldStructString, dstr[:PRI.TaskStandardFieldStructSize]
        )
...
class ExtraBlockRecordFactory(object):
    ...
    def recordFromByteArray(self, raw):
        ...
        if len(raw) < self.__packSize:
            raise IOError(
                "Error: raw data passed in is too small; required (%d), available (%d)" % (self.__packSize, len(raw)))
