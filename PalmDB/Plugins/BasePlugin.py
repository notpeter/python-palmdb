...
class BasePDBFilePlugin:
    ...
    def loadFromApplicationFile(self, PalmDB, applicationID, filename):
        if applicationID not in self.getSupportedDesktopApplications(READ):
            raise NotImplementedError(
                'This plugin does not support [%s; %s]' % (
                    applicationID, getDesktopApplicationNameFromID(applicationID))
            )
    ...
    def unpackXMLFromFile(self, applicationID, filename):
        # +++ READ THIS +++ You should not be trying to decode the XML,
        # it has to be a stream of bytes, the XML parser needs to do the decoding.
